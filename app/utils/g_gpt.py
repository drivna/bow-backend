...
def _normalize_openai_messages_to_gemini(message_for_model: List[Dict[str, Any]]):
    """
    Convert OpenAI-style messages [{role:'user'|'assistant'|'system', content:str|list}]
    into Gemini's (system_instruction, contents) format.
    ...
    def _content_to_text(c):
        # Handle either str or list of dicts (OpenAI can send tool messages etc.; we collapse to text)
        if isinstance(c, str):
            return c
        if isinstance(c, list):
            # Join any text-like parts; ignore non-text parts for simplicity
            parts = []
            for p in c:
                ...
                if isinstance(p, dict):
                    if "text" in p:
                        ...
                        parts.append(p["text"])
                    # Otherwise skip non-text (images, tools, etc.)
            return "\n".join(parts).strip()
        # Fallback
        return str(c)
    ...
    for m in message_for_model:
        ...
        if role == "system":
            # Prefer the first system message as system_instruction
            if not system_instruction:
                system_instruction = text
            else:
                # If multiple system messages, append them together.
                system_instruction += ("\n" if system_instruction else "") + text
            continue

        # Gemini expects 'user' or 'model' roles in contents
...
def fetch_response_from_model(message_for_model: List[Dict[str, Any]]):
    try:
        ...
        # Gemini returns text content in response.text
        content: str = response.text or ""

        # Preserve your code-fence stripping before json.loads
